...
def create(_Product: Product) -> int:
    new_id = dbmanager.execute(
        query="insert into Product(userId,discription,price) values(?,?,?)",
        args=(_Product.userId,_Product.discription,_Product.price)
    )
    return new_id
...
def update(Product_id: int, new_data: Product) -> None:
    return dbmanager.execute(
        query='update Product set (userId,discription,price) = (?,?,?) where id=(?)',
        args=(new_data.userId,new_data.discription,new_data.price, Product_id))
